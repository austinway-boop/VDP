class AudioRecorder {
    ...
    displayWordBreakdown(wordAnalysis) {
        ...
        if (!wordAnalysis || wordAnalysis.length === 0) {
            this.elements.wordBreakdown.innerHTML = '<span class="no-words">No words to analyze</span>';
            return;
        }
        
        wordAnalysis.forEach(wordData => {
            const wordSpan = document.createElement('span');
            wordSpan.className = `word-item ${wordData.found ? wordData.emotion : 'not-found'}`;
            wordSpan.textContent = wordData.word;
            
            // Create tooltip with detailed information
            const tooltip = wordData.found 
                ? `Emotion: ${wordData.emotion} (${Math.round(wordData.confidence * 100)}%)\nValence: ${wordData.valence.toFixed(2)}\nArousal: ${wordData.arousal.toFixed(2)}\nSentiment: ${wordData.sentiment}`
                : `Word not found in emotion database`;
